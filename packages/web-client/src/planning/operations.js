...
export const refreshMembersSchedules =
  (year, weekNumber, callback) => async (dispatch) => {
    try {
      const schedules = await api.get(`/api/schedules/${year}-${weekNumber}`);
      dispatch(
        actions.refreshMembersSchedulesSuccess(year, weekNumber, schedules)
      );
      if (callback) {
        callback();
      }
...
export const submitPersonalSchedule =
  (groupId, memberId, schedule, callback = null) =>
  async (dispatch) => {
    ...
    try {
      ...
      dispatch(
        actions.savePersonalScheduleSuccess(groupId, memberId, schedule)
      );
      if (callback) {
        callback();
      }
...
export const submitDefaultSchedule =
  (groupId, defaultSchedule, weekCursor) => async (dispatch) => {
